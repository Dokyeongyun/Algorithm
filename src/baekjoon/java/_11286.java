    ...
    public static void main(String[] args) throws IOException {
        ...
        for (int i = 0; i < n; i++) {
            ...
            if (temp != 0) {
                ...
                if (plusQue.size() == 0 && minusQue.size() == 0) {
                    sb.append(0 + "\n");
                } else if (plusQue.size() == 0) {
                    sb.append(minusQue.poll()*-1 + "\n");
                } else if (minusQue.size() == 0) {
                    sb.append(plusQue.poll() + "\n");
                } else {
                    if (plusQue.peek() < minusQue.peek()) {
                        sb.append(plusQue.poll() + "\n");
                    } else {
                        sb.append(minusQue.poll()*-1 + "\n");
                    }
